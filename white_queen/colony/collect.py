...
class _RandomAgent:
    ...
    def train(self, env, config, tracker): pass
...
def _make_agent(row, env, seed, steps):
    ...
    cfg = {"seed": seed, "steps": steps, "device": "cpu",
...
def _rollout(agent, env, rng, nA, n_episodes, eps):
    ...
    for _ in range(n_episodes):
        ...
        while not done:
            greedy = int(agent.act(state, eval=True)) if not isinstance(agent, _RandomAgent) \
            ...
            state = ns
        out.append({"obs": np.asarray(o), "act": np.asarray(a), "rew": np.asarray(r),
                    "done": np.asarray(d), "greedy": np.asarray(g), "prob": np.asarray(pr)})
    return out
...
def run(cfg, db_path, ckpt_dir="white_queen_checkpoints"):
    ...
    for row in build_roster(cfg):
        for seed in cfg["seeds"]:
            ...
            if steps:
                agent.train(env, {"seed": seed, "steps": steps, "device": "cpu",
                                  "buffer_size": max(steps * 2, 10_000), **row["extra"]},
                            _NullTracker())
                agent.save(os.path.join(ckpt_dir, f"{row['policy_id']}_s{seed}.pt"))
            pid = f"{row['policy_id']}_s{seed}"
            db.register_policy(con, pid, row["family"], row["key"], frac, seed,
                               steps, cfg["collect_eps"])
            rng = np.random.default_rng(1000 + seed)
            rets = []
            for ep in _rollout(agent, env, rng, nA, cfg["collect_episodes"],
                               cfg["collect_eps"]):
                db.insert_episode(con, pid, seed, ep["obs"], ep["act"], ep["rew"],
                                  ep["done"], ep["greedy"],
                                  np.full_like(ep["prob"], cfg["collect_eps"]), ep["prob"])
