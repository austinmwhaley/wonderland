...
def _bandit_case(seed=0):
    ...
    rb = evaluate(
        logs, best, nA=info["nA"], estimate_propensity=True, fast=True, fqe_cfg=dict(tiny)
    )
    rh = evaluate(
        logs, hold, nA=info["nA"], estimate_propensity=True, fast=True, fqe_cfg=dict(tiny)
    )
    ...
    cells = [
        Cell("best", bool(rb["deploy"]), truth_best, anchor, bar, 1, cb.get("lo"), cb.get("hi")),
        Cell("logging", bool(rh["deploy"]), truth_hold, anchor, bar, 2, ch.get("lo"), ch.get("hi")),
    ]
    return Case(f"bandit_synth#s{seed}", cells, meta={"family": "bandit", "seed": seed})
...
def _matrix_case(env, seed=0):
    ...
    return Case(f"{env}#s{seed}", cells, meta={"family": FAMILY[env], "seed": seed})
...
def _continuous_case(env, seed=0):
    ...
    return Case(
        ...
        meta={"family": "continuous", "seed": seed, "improved": bool(r["improved"])},
    )
...
if __name__ == "__main__":
    ...
    if "--run" in args:
        ...
        seeds = [seed_only] if seed_only is not None else CONTRACT["seeds"]
        for e in envs:
            for sd in seeds:
