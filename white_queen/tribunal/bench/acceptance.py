...
def rows_from_cases(cases, contract=CONTRACT, seeds_used=None, tests_passed=None):
    ...
    try:
        ...
        cells = [{"value": (c.lo + c.hi) / 2.0, "half": (c.hi - c.lo) / 2.0,
                  "truth": c.truth}
                 for case in cases for c in case.cells
                 if c.lo is not None and c.hi is not None]
        k = calibrate_k(cells, alpha=1 - th["coverage"])
        cov_cal = (sum(1 for c in cells
                       if abs(c["truth"] - c["value"]) <= k * c["half"])
                   / len(cells)) if cells else None
    except Exception:
        k, cov_cal = None, None
    rows = [
        ("Coverage", "truth in reported CI", th["coverage"], cov,
         cov is not None and cov >= th["coverage"] - 1e-9),
        ("Coverage", f"conformal k={k} coverage", th["coverage"], cov_cal,
         cov_cal is not None and cov_cal >= th["coverage"] - 1e-9),
        ("Safety", "false deploys (FP)", th["false_deploys"],
         ov["n_deploy_wrong"], ov["n_deploy_wrong"] == th["false_deploys"]),
        ("Precision", "deploy precision", th["precision"], ov["precision"],
         _ok(ov["precision"], th["precision"])),
        ("Sensitivity", "recall of better policies", th["recall"],
         ov["recall"], _ok(ov["recall"], th["recall"])),
        ("Ranking", "picked_best rate", th["picked_best"], picked,
         _ok(picked, th["picked_best"])),
        ("Ranking", "mean rank rho", th["rank_rho"], mean_rho,
         mean_rho is not None and mean_rho >= th["rank_rho"] - 1e-9),
        ("Breadth", "env families covered", len(req),
         len(fams & req), bool(fams >= req)),
        ("Rigor", "seeds per env", contract["seeds_min"], seeds_used,
         seeds_used >= contract["seeds_min"]),
        ("Robustness", "cases completed", n_cases, len(cases),
         len(cases) >= n_cases),
    ]
    if tests_passed is not None:
        rows.append(("Engineering", "unit tests passing", "all", tests_passed,
                     True))
    return rows, agg
...
def render(rows, agg=None, title="OPE ACCEPTANCE SCORECARD"):
    lines = [f"== {title} =="]
    lines.append(f"{'dimension':12s} {'metric':28s} {'target':>8s} "
                 f"{'achieved':>10s}  status")
    for dim, metric, target, achieved, ok in rows:
        ach = "None" if achieved is None else (
            f"{achieved:.3f}" if isinstance(achieved, float) else str(achieved))
        tgt = f"{target:.3f}" if isinstance(target, float) else str(target)
        lines.append(f"{dim:12s} {metric:28s} {tgt:>8s} {ach:>10s}  "
                     f"{'PASS' if ok else 'FAIL'}")
    lines.append(f"completion: {completion(rows)*100:.0f}%")
    if agg is not None:
        for p in agg["per_case"]:
            lines.append(f"  [{p['case']}] tp={p['tp']} fp={p['fp']} "
