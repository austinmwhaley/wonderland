...
from algorithms.base import evaluate  # noqa: E402
...
def _discounted_rollout(cand, env, episodes, gamma, proxy_temp=None, seed=0,
                        max_steps=None):
...
def run_diet(cfg, db_path, diet, out_dir):
    ...
    for name in CANDIDATES:
        ...
        try:
            ...
            pass
        p = ope.panel(data, cand, cfg["gamma"], cfg.get("ope_meta"), fqe_base,
                      cand_id=f"{name}_{diet}", cache_dir=_cdir,
                      weights_hash=_wh, fast=cfg.get("fast", False),
                      ensemble_K=cfg.get("ensemble_K"))
        p["truth"] = _discounted_rollout(cand, env, gt_eps,
                                         cfg["gamma"])
        p["proxy_truth"] = _discounted_rollout(
            cand, env, gt_eps, cfg["gamma"],
            proxy_temp=p["temperature"])
        panels[name] = p
        log.info(f"  {name:4s} blend={p['blended']:7.1f} DR={p['dr']:7.1f} "
        ...
        log.info(f"         WIS={p['wis']:7.1f} FQE={p['fqe_dm']:7.1f} "
    ...
    rows = gate_mod.adjudicate(panels, bstat["mean"], bstat["std"],
                               cfg.get("gate"), cfg.get("ope_meta"),
                               n_episodes=n_ep)
    order = list(panels)
    rho = spearman([panels[c]["blended"] for c in order],
                   [panels[c]["truth"] for c in order])
    ...
    judge_v = judge_mod.judge_diet(rows, bstat["mean"], bstat["std"],
                                   cfg.get("gate"),
                                   cfg.get("risk_aversion",
                                           judge_mod.DEFAULT_RISK_AVERSION),
    ...
    report = {"diet": diet, "behavior_mean": round(bstat["mean"], 1),
              "gate_cfg": {"configured": cfg.get("gate"),
              ...
              "deployed": deployed, "true_best": best_true,
              "gate_picked_best": best_true in deployed,
              "gate_deployed": gate_deployed,
              "decided_by": "judge",
              "judge": judge_v,
              "judge_rationale": judge_mod.explain_diet(
                  diet, bstat["mean"], judge_v, rows)}
    with open(os.path.join(out_dir, f"verdict_{diet}.json"), "w") as f:
        json.dump(report, f, indent=2)
    log.info(f"  verdict: deployed={deployed} true_best={best_true} "
...
def run(cfg, db_path, out_dir="white_queen_verdicts"):
    ...
    for d, r in reports.items():
        ...
        log.info(f"[{d}] {marks} spearman={r['spearman_blended_truth']:.3f} "
              f"picked_best={r['gate_picked_best']}")
    return reports


def run_diet_ope_only(cfg, db_path, diet, out_dir, ckpt_dir,
                      mu_source="logged"):
    ...
    if mu_source == "estimated":
        ...
        log.info(f"industry mu-hat: acc={mu_info['behavior']['accuracy']} "
    ...
    for name in _names:
        ...
        try:
            ...
            pass
        _pk = {"cand_id": f"{name}_{diet}", "cache_dir": _cache_dir,
               "weights_hash": _wh}
        if _tier == "screen":
            # Screen tier: small explicit budgets; may over-hold, never deploy.
            fqe_base = dict(fqe_base, steps_max=2000, eval_every=200,
                            patience=2)
            p = ope.panel(data, cand, cfg["gamma"], {"bootstrap_B": 200},
                          fqe_base, ensemble_K=2, dice_steps=1000,
                          magic_B=200, fast=cfg.get("fast", False), **_pk)
        else:
            p = ope.panel(data, cand, cfg["gamma"], cfg.get("ope_meta"),
                          fqe_base, fast=cfg.get("fast", False),
                          ensemble_K=cfg.get("ensemble_K"), **_pk)
        p["truth"] = _discounted_rollout(cand, env, gt_eps, cfg["gamma"])
        p["proxy_truth"] = _discounted_rollout(cand, env, gt_eps, cfg["gamma"],
                                               proxy_temp=p["temperature"])
        panels[name] = p
        log.info(f"  {name:4s} blend={p['blended']:7.1f} DR={p['dr']:7.1f} "
...
def run_ope_only(cfg, db_path, out_dir, ckpt_dir, mu_source="logged"):
    reports = {d: run_diet_ope_only(cfg, db_path, d, out_dir, ckpt_dir,
                                    mu_source=mu_source)
               for d in cfg["diets"]}
