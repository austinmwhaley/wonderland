...
def learn_ratio(
    ...
    if cand_id is not None and cache_dir:
        ...
        _ckey = make_key(
            "mis", diet_hash(diet), cand_id, ac, temperature, weights_hash or "noweights"
        )
    ...
    if _cache_hit:
        ...
        _gov = govern(
            ...
            _step,
            None,
            {"steps_max": steps, "eval_every": max(steps // 20, 100), "lr": w_lr, "patience": 0},
        )
        if _ckey is not None:
            from .cache import save as _csave

            _csave(cache_dir, _ckey, {"w": w_net.state_dict(), "nu": nu_net.state_dict()})
...
def support_stats(diet, w_fn):
    ...
    return {
        "w_p50": round(float(np.median(w)), 3),
