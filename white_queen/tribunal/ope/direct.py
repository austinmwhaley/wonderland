...
def ensemble_fqe(diet, cand, gamma, fqe_cfg=None, temperature=None, K=None,
    ...
    for k in range(K):
        ...
        qnet, dm, info = fit_fqe(diet, cand, gamma, cfg, temperature,
    ...
    return {"mean": float(np.mean(means)), "disagreement": float(np.std(means)),
            "members": [round(m, 1) for m in means], "nets": nets, "K": K,
            "val_weighted_mean": vwmean,
            "val_weights": [round(float(x), 3) for x in
                            (w.tolist() if w is not None else [1.0 / K] * K)]}
...
def lstdq(diet, cand, gamma, temperature=1.0, ridge=None, seed=0):
    ...
    for ep in episodes(diet):
        ...
        f0 = np.concatenate([ep["obs"][:1].astype(np.float64),
                             np.eye(nA)[[np.argmax(pi0)]]], 1)
        q_all = np.stack([np.concatenate(
            [ep["obs"][:1].astype(np.float64), np.eye(nA)[[a]]], 1) @ theta
            for a in range(nA)])
        vals.append(float((pi0 * q_all.squeeze(1)).sum()))
    return {"dm": float(np.mean(vals)), "cond": cond,
...
def fve(diet, cand, gamma, fqe_cfg=None, temperature=1.0, rho_cap=None,
        cand_id=None, cache_dir=None, weights_hash=None):
    ...
    patience, eval_every = cfg["patience"], cfg["eval_every"]
    ...
    if rho_cap is None:
        ...
        try:
            ...
            import warnings
            warnings.warn(f"fve rho_cap derivation failed ({e!r}); "
                          f"using fallback 10.0", RuntimeWarning, stacklevel=2)
    ...
    rho_all = np.clip(
        ...
        np.maximum(_mu_take, PROB_FLOOR), 0.0, rho_cap)
    ...
    # (1-Rho)*V(O) — the net's own output — so an untrained near-constant net
    ...
    MC = torch.as_tensor(_mc_returns(diet, gamma).astype(np.float32)
                         ).unsqueeze(1).to(device)

    def v_target(idx):
        with torch.no_grad():
            one_step = R[idx] + gamma * (1 - D[idx]) * vnet(O2[idx])
            return Rho[idx] * one_step + (1 - Rho[idx]) * vnet(O[idx])

    def val_err():
        with torch.no_grad():
            return float(F.mse_loss(vnet(O[va]), MC[va]))
    ...
    def _step(n, lr):
        ...
        for _ in range(n):
            ...
            loss = F.mse_loss(vnet(O[i]), v_target(i))
            opt.zero_grad()
            loss.backward()
            opt.step()
    ...
    if cand_id is not None and cache_dir:
        ...
        _ckey = make_key("fve", diet_hash(diet), cand_id, cfg, temperature,
                         weights_hash or "noweights")
        _hit = _cload(cache_dir, _ckey, map_location=device)
    if _hit is not None:
        vnet.load_state_dict(_hit["v"])
        _gov = {"steps": 0, "n_evals": 0, "best_val": None,
                "stopped": "cache-hit", "lr_final": lr}
        _cache_hit = True
    else:
    ...
    return {"dm": float(np.mean(vals)), "steps": _gov["steps"],
            "stopped": _gov["stopped"], "device": device,
