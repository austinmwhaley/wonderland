...
_TINY_FQE = {"steps_max": 2000, "eval_every": 500, "patience": 4,
             "batch": 256, "hidden": 64,
...
def test_register_custom_trainer():
    ...
    rep = run(logs, algorithms=("iql", "const0"), nA=info["nA"],
              estimate_propensity=True, offline_steps=2000, fast=True,
              ensemble_K=2, fqe_cfg=dict(_TINY_FQE),
...
def test_external_policy_object_directly():
    ...
    optimal.name = "oracle"
    rep = run(logs, algorithms=("bc", optimal), nA=info["nA"],
              estimate_propensity=True, offline_steps=1500, fast=True,
              ensemble_K=2, fqe_cfg=dict(_TINY_FQE),
