...
def _row(**kw):
    base = {
        "fqe_dm": 40.0, "sharp_dm": 40.0, "sharp_info": {},
        "mb": {"mb": 40.0, "se": 1.0}, "mb_se": 1.0,
        "mb_sharp": 40.0, "mb_sharp_se": 1.0,
        "efqe": {"mean": 40.0, "disagreement": 1.0},
        "dr": 40.0, "dr_ci": [30.0, 50.0], "wis": 40.0, "is": 40.0,
        "lstdq": 40.0, "level_est": 40.0, "ess_frac": 0.2,
        "fqe_info": {}, "dr_step_adv": 0.0,
    }
    base.update(kw)
    return base
...
def test_same_source_cannot_corroborate():
    ...
    row = _row(fqe_dm=90.0, sharp_dm=90.0, mb={"mb": 10.0, "se": 1.0},
               mb_sharp=10.0, dr=10.0, dr_ci=[0.0, 20.0])
...
def test_disjoint_sources_corroborate():
    ...
    row = _row(fqe_dm=90.0, sharp_dm=95.0, mb={"mb": 90.0, "se": 1.0},
               mb_sharp=95.0, mb_sharp_se=1.0, ess_frac=0.0)
    est = K.estimates_from_row(row, bar=60.0, min_ess=0.02)
...
def test_under_budget_fqe_cannot_gate():
    row = _row(fqe_dm=90.0, sharp_dm=95.0, mb={"mb": 90.0, "se": 1.0},
               mb_sharp=95.0, fqe_info={"under_budget": True})
    est = {e.name: e for e in K.estimates_from_row(row, bar=60.0, min_ess=0.02)}
    assert est["fqe_argmax"].reliable is False
...
def test_diverged_fqe_excluded_but_reason_reported():
    row = _row(fqe_dm=None, sharp_dm=None,
               sharp_info={"diverged": True, "reason": "holdout Bellman 9>8"},
               mb={"mb": 90.0, "se": 1.0}, mb_sharp=95.0)
