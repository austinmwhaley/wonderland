...
def test_candidate_precompute_surgery_engaged():
    ...
    E.fit_fqe(_diet(), _CountingUniform(), 0.99,
              {"steps_max": 120, "eval_every": 40, "patience": 5, "batch": 32,
               "hidden": 16, "device": "cpu"}, temperature=1.0,
              cand_id="u", cache_dir=None)
...
def test_fqe_cache_hit_skips_training():
    ...
    torch.manual_seed(0)
    torch.set_num_threads(1)
    tiny = {"steps_max": 120, "eval_every": 40, "patience": 5, "batch": 32,
            "hidden": 16, "device": "cpu"}
    with tempfile.TemporaryDirectory() as td:
        _, dm1, i1 = E.fit_fqe(_diet(), _CountingUniform(), 0.99, dict(tiny),
                               temperature=1.0, cand_id="u",
                               cache_dir=td, weights_hash="w")
        assert not i1["cache_hit"] and i1["steps"] > 0
        _, dm2, i2 = E.fit_fqe(_diet(), _CountingUniform(), 0.99, dict(tiny),
                               temperature=1.0, cand_id="u",
                               cache_dir=td, weights_hash="w")
...
def test_screen_tier_contests_never_deploys():
    ...
    tiny = {"steps_max": 120, "eval_every": 40, "patience": 2, "batch": 32,
            "hidden": 16, "device": "cpu"}
    p = E.panel(_diet(), _CountingUniform(), 0.99,
                meta={"bootstrap_B": 40, "temps": (0.5, 1.0)},
                fqe_cfg=tiny, ensemble_K=2, dice_steps=100, magic_B=40)
    rows = {"u": {"fqe_dm": p["fqe_dm"], "dr": p["dr"], "dr_ci": [0.0, 1.0],
                  "efqe": p["efqe"], "ess_frac": 0.5, "wis": 0.0,
                  "mb": p["mb"], "temperature": 1.0}}
    v = J.judge_diet(rows, 0.0, 20.0, {"rel_edge_std": 0.2,
                                       "min_ess_frac": 0.01},
                     risk_aversion=0.0, allow_deploy=False)
