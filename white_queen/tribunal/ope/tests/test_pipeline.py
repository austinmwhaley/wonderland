...
_TINY_FQE = {"steps_max": 8000, "eval_every": 500, "patience": 5,
             "batch": 256, "hidden": 64,
...
def test_pipeline_trains_and_decides():
    ...
    logs, info = make_sequential(n=6000, d=6, nA=4, T=15, seed=0,
                                 behavior_eps=0.5)
    rep = run(logs, gamma=0.99, nA=info["nA"], estimate_propensity=True,
              offline_steps=6000, fast=True, ensemble_K=2,
              fqe_cfg=dict(_TINY_FQE), out_dir="/tmp/opencode/wq_pipe")
    ...
    for n in rep["rank"]:
        e = rep["estimates"][n]
        print(f"  {n}: FQE-soft={e['fqe_dm']} FQE-argmax={e['sharp_dm']} "
...
def test_pipeline_rejects_bandit_logs():
    ...
    logs, info = make_bandit(n=500, d=4, nA=3, seed=0,
