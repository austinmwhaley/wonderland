...
def train_candidates(data, algorithms=("iql", "cql", "bc"), *, gamma=0.99,
                     offline_steps=None, seed=0, out_dir=None, cfg=None):
    ...
    for i, entry in enumerate(algorithms):
        ...
        if entry not in CANDIDATES:
            ...
            if (entry not in CANDIDATE_TRAINERS and
                    entry not in ("random", "iql_cont", "bc_cont")):
                raise ValueError(
...
def evaluate_pool(data, handles, *, gamma=0.99, fast=True, ensemble_K=None,
                  fqe_cfg=None, risk_aversion=0.5, cache_dir=None,
                  behavior_cfg=None):
    ...
    from .receipts import behavior_stats, spearman
    ...
    for name in names:
        if data.get("continuous"):
            ...
            p = panel_continuous(data, handles[name], gamma, fqe_cfg=fqe_cfg,
                                 fast=fast, cand_id=f"{name}",
                                 cache_dir=cache_dir)
        else:
            p = E.panel(data, handles[name], gamma, meta=None, fqe_cfg=fqe_cfg,
                        cand_id=f"{name}", cache_dir=cache_dir,
                        ensemble_K=ensemble_K, fast=fast)
        panels[name] = p
        per[name] = p
    rows = G.adjudicate(panels, b["mean"], b["std"], None, None,
                        n_episodes=len(np.unique(data["episode"])))
    verdict = J.judge_diet(rows, b["mean"], b["std"], None,
                          risk_aversion=risk_aversion)
    report = {
        "behavior_mean": round(b["mean"], 2), "behavior_std": round(b["std"], 2),
        "bar": verdict["bar"], "rank": verdict["rank"],
        "deployed": verdict["deployed"],
        "decisions": {n: verdict["decisions"][n] for n in names},
        "estimates": {n: {k: rows[n].get(k) for k in
                          ("fqe_dm", "sharp_dm", "mb", "mb_sharp", "dr",
                           "dr_ci", "level_est", "ess_frac", "lstdq", "wis",
                           "is", "dr_step", "step_ess_frac")} for n in names},
...
def run(source, *, algorithms=("iql", "cql", "bc"), gamma=0.99,
        columns=None, nA=None, estimate_propensity=True, behavior_cfg=None,
        behavior_seed=0, offline_steps=None, seed=0, fast=True, ensemble_K=None,
        fqe_cfg=None, risk_aversion=0.5, cache_dir=None, out_dir=None,
        source_name=None):
    ...
    d = _data.to_canonical(source, columns=columns, nA=nA,
    ...
    validate_diet(d)
    if d["mode"] != "rl":
        raise ValueError("offline-RL pipeline needs sequential logs "
                         "(next_obs + done). For bandit logs use evaluate().")
    handles = train_candidates(d, algorithms, gamma=gamma,
                               offline_steps=offline_steps, seed=seed,
                               out_dir=out_dir)
    return evaluate_pool(d, handles, gamma=gamma, fast=fast,
                         ensemble_K=ensemble_K, fqe_cfg=fqe_cfg,
                         risk_aversion=risk_aversion, cache_dir=cache_dir,
                         behavior_cfg=behavior_cfg)
