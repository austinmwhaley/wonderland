...
def drift_report(diet, gamma=0.99, n_periods=4):
    ...
    obs_shift = float(np.max(np.abs(obs[parts[-1]].mean(0) - obs[parts[0]].mean(0)) / obs_sd))
    ...
    return {
        ...
        "note": (
            "pool is non-stationary — recompute behavior/conclusions per period rather than pooling"
            if drift_flag
