"""DuckDB colony store. Two tables + coverage-diet views. The DB is the toy event
stream: episodes = sessions, transitions = events, policies = behavior actors.

Storage is a native DuckDB file (``.duckdb``) — Arrow/DuckDB is the project's
only storage stack; there is no SQLite anywhere.

Observations are stored as explicit DOUBLE columns sized to obs_dim (see
...
_STATEMENTS = [
    "CREATE SEQUENCE IF NOT EXISTS episodes_id_seq",
    "CREATE SEQUENCE IF NOT EXISTS transitions_tid_seq",
    ...
    """CREATE TABLE IF NOT EXISTS policies (
        ...
        checkpoint_frac DOUBLE,      -- NULL for random; else fraction of training
        seed        BIGINT NOT NULL,
        train_steps BIGINT NOT NULL,
        collect_eps DOUBLE NOT NULL
    )""",
    """CREATE TABLE IF NOT EXISTS episodes (
        episode_id BIGINT PRIMARY KEY DEFAULT nextval('episodes_id_seq'),
...
def _schema(obs_dim):
    ocols = ", ".join(f"o{i} DOUBLE" for i in range(obs_dim))
    ncols = ", ".join(f"n{i} DOUBLE" for i in range(obs_dim))
    return _STATEMENTS + [
        f"""CREATE TABLE IF NOT EXISTS transitions (
        tid BIGINT PRIMARY KEY DEFAULT nextval('transitions_tid_seq'),
        ...
        greedy_action BIGINT NOT NULL,
        eps DOUBLE NOT NULL,
        prob_taken DOUBLE NOT NULL
    )"""
    ]
...
def init_db(path, obs_dim=4, nA=2, gamma=0.99):
    con = connect(path)
    for stmt in _schema(int(obs_dim)):
        con.execute(stmt)
    con.executemany(
    ...
    for name, sql in VIEWS.items():
        con.execute(f"CREATE OR REPLACE VIEW diet_{name} AS {sql}")
    return con
...
def insert_episode(
    ...
    eid = con.execute(
        "INSERT INTO episodes (policy_id, seed, ret, length) VALUES (?,?,?,?) RETURNING episode_id",
...
def load_diet(path, diet):
    ...
    Native DuckDB read of the colony file: the step-within-episode ``t`` is a
    vectorized window function; fetch goes through Arrow/zero-copy numpy.
    """
    if not Path(path).exists():
        raise FileNotFoundError(
            f"colony db not found: {path} (generate it with: python -m white_queen.run_colony)"
        )
    ...
    try:
        ...
        q = (
            ...
            f"FROM diet_{diet} ORDER BY tid"
        )
