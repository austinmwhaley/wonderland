...
def run(seed=0):
    ...
    te = np.array([kk in test_c for kk in k])
    tr = ~te
    ...
    def uplift(S_):
        f1 = feats(S_, np.ones(len(S_)))
        f0 = feats(S_, np.zeros(len(S_)))
    ...
    boot = [
        float(uplift(S[te])[j].mean())
        for j in (rng.integers(0, te.sum(), te.sum()) for _ in range(300))
    ]
    ...
    res = {
        "customer_periods": int(len(y)),
        "treated": int((t == 1).sum()),
        "heldout": int((t == 0).sum()),
        "customers": int(len(uniq)),
        "ATE_incremental_margin": round(ate, 2),
        "ate_ci95": [round(lo, 2), round(hi, 2)],
