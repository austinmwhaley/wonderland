...
def _best_joint(channel):
    ...
    try:
        df = con.execute(
			...
			FROM contact_sends cs LEFT JOIN orders o ON o.transaction_id=cs.converted_order_id
			WHERE cs.channel=? AND cs.arm IS NOT NULL""",
            [channel],
        ).pl()
    finally:
        con.close()
...
def build_plan(cadence="weekly", budget=None, seed=0):
    ...
    for k in responders:
        ...
        for ch, cad in CHANNEL_CADENCE.items():
            ...
            for j in range(n):
                ...
                contacts.append({"channel": ch, "ts": ts, "discount_pct": float(best_disc[ch])})
            used += n
        if contacts:
    ...
    report = {
        ...
        "included_channels": [
            ch for ch in CHANNEL_CADENCE if (not cert_path.exists() or certified.get(ch, False))
        ],
        "send_hour": hours,
