...
def run(seed=0, global_budget=None):
	...
	z = np.load(LOG)
	S = z["state"]; S2 = z["next_state"]; R = z["reward"]; D = z["done"]
	n = z["action"][:, 0]
	...
	diet = _data.to_canonical({"obs": S[sub], "act": act[sub], "rew": R[sub],
							   "next_obs": S2[sub], "done": D[sub]}, nA=nA)
	...
	if not deployed:
		...
		for i in range(len(S)):
			...
			used += take
			plan.append({"epoch": int(i), "cadence": ep_cad[int(z["cadence"][i])],
						 "action_bucket": a, "actions": take})
			if used >= global_budget:
				break
	OUT.parent.mkdir(parents=True, exist_ok=True)
	OUT.write_text(json.dumps({"deployed": deployed, "budget": global_budget,
							   "used": used, "plan": plan}))
	by_cad = {}
	for p in plan:
