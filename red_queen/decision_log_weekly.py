...
def build():
    ...
    rows = list(dense.iter_rows(named=True))
    i, n = 0, len(rows)
    ...
    np.savez_compressed(
        ...
        customer=np.array(CUST),
        epoch_ts=np.array(ET0, np.float64),
    )
    return {
        "steps": len(R),
        "trajectories": tid,
        "cadence_mix": {
            c: int((np.array(CAD) == i).sum()) for i, c in enumerate(("daily", "weekly", "monthly"))
        },
        "out": str(OUT),
    }
