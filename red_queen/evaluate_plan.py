...
def evaluate(risk_aversion_z=1.96, seed=0):
	...
	a = df["arm"].to_numpy(); e = df["propensity"].to_numpy(); r = df["g"].to_numpy()
	pi = np.array([pmap.get(k, -1) for k in keys])
	served = pi >= 0
	# estimate per-arm ground truth E[r|do(arm)] via IPW (for best-constant reference)
	nA = int(a.max()) + 1
	gt = np.array([float((r[a == x] / e[a == x]).sum() /
						 max((1 / e[a == x]).sum(), 1e-9)) for x in range(nA)])
	...
	return {"served": int(len(ix)), "n_total": int(len(keys)), "nA": nA,
			"gt_per_arm": [round(x, 2) for x in gt], "best_arm_by_gt": best_arm,
			"behavior_value": round(v_beh, 2), "plan_value_ips": round(v_plan, 2),
			"best_constant_value_ips": round(v_best, 2),
			"lift": round(v_plan - v_beh, 2), "lift_ci95": [round(lo, 2), round(hi, 2)],
