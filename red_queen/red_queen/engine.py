...
def _value_model():
	...
	for a in range(nA):
		...
		if m.sum() > 50:
			...
			sig = float(R.std())
		models.append(mod); sigmas.append(sig)
...
def _validated_arm_effects():
	...
	import duckdb
	import polars as pl
	con = duckdb.connect(str(WORK / "rabbit_hole" / "data" / "duckdb"
							 / "customer_event_stream.duckdb"), read_only=True)
	try:
	...
	return np.array([float((g[a == x] / e[a == x]).sum() / max((1 / e[a == x]).sum(), 1e-9))
...
def run(weekly_budget=None, risk_z=0.0, use_red_king=False):
	...
	if use_red_king:
		...
		V = np.zeros((len(keys), nA)); V[:, best_a] = who
		SD = np.zeros_like(V)
	...
	cad = np.where(vbest >= q[1], "weekly",
				   np.where(vbest >= q[0], "biweekly", "monthly"))
	cost = CADENCE[best]
	if weekly_budget is None:
		weekly_budget = float(len(keys) * CADENCE.mean())
	order = np.argsort(-(score[np.arange(len(keys)), best] * lb_ok))
	arm = np.full(len(keys), -1, int); budget = weekly_budget; budget_rej = 0
	for i in order:
		if not lb_ok[i]:
			continue
		if cost[i] <= budget:
			arm[i] = best[i]; budget -= cost[i]
		else:
			budget_rej += 1
	served = arm >= 0
	...
	plan = {"customer_key": keys, "arm": np.where(served, arm, -1).tolist(),
			"cadence": cad.tolist(), "weekly_sends": sends.tolist(),
	...
	rec = {"customers": len(keys),
		   ...
		   "cadence_mix": {c: int((cad[served] == c).sum()) for c in ("weekly", "biweekly", "monthly")},
