...
def run(seed=0):
	import polars as pl
	gr = load()
	y = gr["gm"].to_numpy(); h = gr["holdout"].to_numpy().astype(bool); k = gr["k"].to_numpy()
	treated, held = ~h, h
	...
	X = np.stack([emap[t] for t in ks]); yv = np.array([incr[t] for t in ks])
	...
	return {"customer_periods": len(y), "treated": int(treated.sum()), "heldout": int(held.sum()),
			"population_incrementality": round(pop, 2), "pop_ci95": [round(lo, 2), round(hi, 2)],
