...
def run(global_budget=None, seed=0):
	z = np.load(LOG)
	S = z["state"]; R = z["reward"]; TID = z["traj"]; DT = z["dt_days"]; CAD = z["cadence"]
	...
	for t in np.unique(TID):
		...
		for i in idx:
			...
			for gi in np.argsort(-vg):
				...
				if vg[gi] > 0 and n <= (global_budget - used) and n <= PER_EPOCH_CAP:
					best_n, best_v = n, float(vg[gi]); break
			used += best_n
			plan.append({"epoch": int(i), "cadence": episode_cad[int(CAD[i])],
		...
		if used >= global_budget:
			break
	plan_arr = {k: [p[k] for p in plan] for k in plan[0]} if plan else {}
