...
def run(seed=0, max_steps=12000):
	import polars as pl
	z = np.load(LOG)
	S = z["state"]; S2 = z["next_state"]; R = z["reward"]; D = z["done"]; A = z["action"][:, 0]
	act = _discretize(A)
	nA = int(act.max()) + 1
	idx = np.arange(len(S))
	if len(idx) > max_steps:
		rng = np.random.default_rng(seed); idx = rng.choice(idx, max_steps, replace=False)
	src = {"obs": S[idx], "act": act[idx], "rew": R[idx],
		   "next_obs": S2[idx], "done": D[idx]}
	from white_queen.tribunal.ope.pipeline import run as wq_run
	rep = wq_run(src, algorithms=("iql", "cql", "bc"), nA=nA, fast=True,
	...
	return {"deployed": rep.get("deployed"), "behavior": rep.get("behavior_mean"),
