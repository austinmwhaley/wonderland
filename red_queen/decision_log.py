	...
	while i < n:
		...
		if len(grp) >= 2:
			for m in range(len(grp) - 1):
				...
				A.append(act); R.append(float(ig[(it > t0) & (it <= t1)].sum()) if len(io) else 0.0)
				dt = (t1 - t0) / 86400.0
				DT.append(dt); CAD.append(_cadence(dt))
				D.append(1.0 if m == len(grp) - 2 else 0.0); TID.append(tid)
			tid += 1
		i = j
	out = {"state": np.array(S, np.float32), "next_state": np.array(S2, np.float32),
		   "action": np.array(A, np.float32), "reward": np.array(R, np.float32),
		   "dt_days": np.array(DT, np.float32), "cadence": np.array(CAD, np.int64),
		   "done": np.array(D, np.float32), "traj": np.array(TID, np.int64)}
	OUT.parent.mkdir(parents=True, exist_ok=True)
	np.savez_compressed(OUT, **out)
	return {"steps": len(R), "trajectories": tid, "state_dim": len(S[0]),
			"action_dim": ACTION_DIM,
			"cadence_mix": {c: int((out["cadence"] == i).sum()) for i, c in enumerate(("daily", "weekly", "monthly"))},
			"mean_actions_per_epoch": round(float(out["action"][:, 0].mean()), 2),
			"max_actions_per_epoch": int(out["action"][:, 0].max()),
			"reward_mean": round(float(out["reward"].mean()), 2), "out": str(OUT)}
