...
def run(window_days: int = 365, seed: int = 0):
    ...
    spec = PluginSpec(
        name=f"customer_segmentation_{window_days}d",
        kind="unsupervised",
        target="gross_margin",
    ...
    def eta2_of(lab):
        k = int(lab.max()) + 1
        overall = ds.y.mean()
        ssb = sum(len(ds.y[lab == c]) * (ds.y[lab == c].mean() - overall) ** 2 for c in range(k))
    ...
    eta2 = eta2_of(lab)
    p = (
        float(f_oneway(*[g for g in groups if len(g) > 1]).pvalue)
    ...
    save_artifact(
        spec,
        {
            ...
            "silhouette": sil,
            "sizes": sizes,
            "segment_mean_gross_margin": means,
            "eta_squared": eta2,
            "anova_p": p,
            "verdict": bool(ok),
        },
