...
def _sender_counts(ds, stream_db=STREAM_DB):
    ...
    try:
        con.register(
            "rh_anchors", pl.DataFrame({"customer_key": ds.keys, "anchor_epoch": ds.anchor_epoch})
        )
        ...
        con.close()
    m = {(r["customer_key"], r["anchor_epoch"]): r["n_sends"] for r in out.iter_rows(named=True)}
    return np.array([m.get((k, a), 0) for k, a in zip(ds.keys, ds.anchor_epoch)], dtype=np.int64)
...
def _fit_reward_policy(X, act, y, nA, seed=0):
    ...
    for a in range(nA):
        ...
        if m.sum() < 10:
            ...
            continue
        W[a] = (
            Ridge(alpha=1e-3 * float(np.mean(np.sum(X[m] ** 2, axis=0))) + 1e-9)
            .fit(X[m], y[m])
            .coef_
        )
...
def run(window_days: int = 365, seed: int = 0):
    ...
    spec = PluginSpec(
        name=f"email_frequency_policy_{window_days}d",
        kind="white_queen",
        target="gross_margin",
    ...
    rows = [
        {
            ...
            "achieved": rep.get("deploy"),
            "ok": rep.get("deploy") in (True, False),
        },
        {
            "check": "behavior value finite",
            "achieved": round(rep["behavior_mean"], 3),
    ...
    save_artifact(
        spec,
        {
            "dataset": ds.meta,
            "n_actions": nA,
            "action_counts": np.bincount(act).tolist(),
            "deploy": rep.get("deploy"),
            "decision": rep.get("decision"),
            "behavior_mean": rep["behavior_mean"],
            "behavior_std": rep["behavior_std"],
            "bar": rep.get("bar"),
            "witnesses": rep.get("witnesses"),
            "value": val,
            "lo": lo,
            "hi": hi,
