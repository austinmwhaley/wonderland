...
def head_point(ds: Dataset, seed=0):
    ...
    return {"name": "point", "pred": pred, "idx": te, "metrics": _metrics(pred, ds.y[te])}
...
def head_two_part(ds: Dataset, seed=0):
    ...
    return {"name": "two_part", "pred": pred, "idx": te, "metrics": _metrics(pred, ds.y[te])}
...
def run(window_days: int, seed: int = 0):
    ...
    heads = [
        head_point(ds, seed),
        head_two_part(ds, seed),
        head_quantile(ds, seed),
        head_baseline(ds, seed),
    ]
    rows = []
    for h in heads:
        m = h["metrics"]
