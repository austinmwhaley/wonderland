...
from algorithms.registry import check_compat, list_implemented
from environments.registry import ENVIRONMENTS
from experiments.configs import DEFAULT_CONFIGS
from experiments.run import RESULTS_DIR, run_experiment, run_id


def main():
    parser = argparse.ArgumentParser(
        description="Run every implemented algorithm x compatible environment."
    )
    ...
    if args.resume:
        ...
        pairs = [
            (a, e) for a, e in pairs if not any(d.startswith(f"{a}_{e}_bench_") for d in existing)
        ]
