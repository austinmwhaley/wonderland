...
cfg=dict(PRESETS['quick']); cfg['device']='cpu'; env=make_env('cartpole',seed=999)
...
for diet in ['mixed','novice_only','expert_only']:
    ...
    for e in np.unique(data['episode']): eplen[e]=int((data['episode']==e).sum())
    ...
    for name in ['iql','bc','cql']:
        ...
        print(f'{diet:12s} {name:4s}: argmax==logged act overall={agree:.3f} on long-exp episodes={agree_long:.3f} (n_long={long_ep.sum()})', flush=True)
