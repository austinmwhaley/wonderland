...
def survey(env_name, seed=0, collect_episodes=25, dqn_steps=20000,
           pg_steps=20000, gt_eps=20, offline_steps=6000):
    try:
        from white_queen.tribunal.ope.training import seed_all as _sa
        import torch as _t
        _sa(seed); np.random.seed(seed); _t.manual_seed(seed)
    ...
    cfg = dict(QUICK_LOOK)
    cfg.update({
        "env": env_name, "seeds": [seed], "collect_episodes": collect_episodes,
        "collect_eps": 0.2, "dqn_train_steps": dqn_steps,
        "pg_train_steps": pg_steps, "checkpoint_fracs": (0.1, 1.0),
        "db_path": db_path, "ckpt_dir": ckpt,
    })
    ...
    d = odata.to_canonical(db_path, estimate_propensity=True,
    ...
    handles = pipeline.train_candidates(
        d, ALGOS, gamma=cfg["gamma"], offline_steps=offline_steps,
        out_dir=cand_dir)
    truth = {n: _discounted_rollout(h, env, gt_eps, cfg["gamma"])
    ...
    rep = pipeline.evaluate_pool(
        d, handles, gamma=cfg["gamma"], fast=True, ensemble_K=_ek,
        fqe_cfg=(_fqe or None))
    ...
    rho_dr = spearman([float(est[n]["dr"]) for n in names],
    ...
    fn = [n for n in names
          if n not in dep and truth[n] > anchor]
    return {
        "env": env_name, "obs_dim": int(d["obs"].shape[1]), "nA": int(d["nA"]),
        "N": int(len(d["obs"])), "behavior_mean": round(anchor, 2),
        "bar": rep["bar"], "t_colony_s": round(t_col, 1),
        "t_ope_s": round(t_ope, 1), "rank": rep["rank"],
        "deployed": rep["deployed"], "true_best": best_true,
        "picked_best": picked_best,
        "rho_sharp": round(rho_sharp, 3), "rho_dr": round(rho_dr, 3),
        "tp": tp, "fp": fp, "fn": fn,
        "certificates": {n: (dec[n].get("certificate") or {}) for n in names},
        "candidates": {
            n: {"truth": round(truth[n], 1),
