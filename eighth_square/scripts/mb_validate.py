...
for diet in ['mixed','novice_only','expert_only']:
    ...
    step_fn, info = learn_dynamics(data, cfg={'steps_max':8000,'eval_every':1000,'patience':3,'batch':512}, cache_dir=None)
    iql=load_candidate('iql', env, data, cfg, f'/home/austin-whaley/wq/white_queen/verdicts/v15/iql_{diet}.pt')
    ...
    print(f'{diet}: uniform MB={ru["mb"]:.1f} (done={ru["done_hits"]}) | iql MB={ri["mb"]:.1f} (done={ri["done_hits"]}) | val_mse={info["val_mse"]:.4f}', flush=True)
