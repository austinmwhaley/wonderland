...
for diet in ['mixed', 'novice_only', 'expert_only']:
    ...
    for name in ['uniform', 'iql']:
        cand = U() if name == 'uniform' else load_candidate(
            'iql', env, data, cfg,
            '/home/austin-whaley/wq/white_queen/verdicts/v15/iql_%s.pt' % diet)
        torch.manual_seed(0)
        _, dm, info = E.fit_fqe(data, cand, 0.99,
                                {'device': 'cuda'}, temperature=1.0)
        print('  %-8s FQE(Polyak) DM=%6.1f tau=%.3f  %s vs bar %.1f' %
              (name, dm, info['target_tau'],
               'PASS' if dm > bar else 'fail', bar), flush=True)
