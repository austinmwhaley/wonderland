...
class MixingNet(nn.Module):
    ...
    def __init__(self, obs_dim, n_agents, hidden=128):
        ...
        self.hyper_w1 = nn.Sequential(
            nn.Linear(obs_dim * n_agents, hidden), nn.ReLU(), nn.Linear(hidden, n_agents * hidden)
        )
        self.hyper_w2 = nn.Sequential(
            nn.Linear(obs_dim * n_agents, hidden), nn.ReLU(), nn.Linear(hidden, hidden)
        )
...
class QMIXAgent(BaseAgent):
    ...
    def __init__(self, env, config):
        ...
        self.qnets = [
            AgentQ(obs_dim, self.n_actions, hidden).to(self.device) for _ in range(self.n_agents)
        ]
        self.qnets_t = [
            AgentQ(obs_dim, self.n_actions, hidden).to(self.device) for _ in range(self.n_agents)
        ]
    ...
    def _learn(self, config, batch_size):
        ...
        q_sel = torch.stack(
            [qs[:, i, :].gather(1, a.unsqueeze(1)).squeeze(1) for i, a in enumerate([a1, a2])],
            dim=1,
        )
        q_tot = self.mixer(q_sel, s_all)
        with torch.no_grad():
            qn = torch.stack(
                [q(ns1) if i == 0 else q(ns2) for i, q in enumerate(self.qnets_t)], dim=1
            )
            a_max = qn.argmax(dim=2)
            qn_sel = torch.stack(
                [
                    qn[:, i, :].gather(1, a_max[:, i].unsqueeze(1)).squeeze(1)
    ...
    def save(self, path):
        torch.save(
            {"qnets": [q.state_dict() for q in self.qnets], "mixer": self.mixer.state_dict()}, path
        )
