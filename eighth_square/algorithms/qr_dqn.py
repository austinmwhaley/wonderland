...
def train_qr_dqn(env: EnvWrapper, config: AlgorithmConfig) -> Result:
    ...
    n_quantiles = 200
    q_net = QuantileQNetwork(env.state_dim, env.action_dim, n_quantiles, config.hidden_dims).to(
        device
    )
    ...
    tau = (
        torch.FloatTensor((2 * np.arange(n_quantiles) + 1) / (2.0 * n_quantiles))
        .view(1, 1, -1)
        .to(device)
    )
    ...
    while episode < config.max_episodes:
        ...
        for step in range(max_steps):
            ...
            if len(buffer) >= config.min_buffer_size:
                ...
                current_q = q_net(states).gather(
                    1, actions.long().unsqueeze(1).unsqueeze(2).expand(-1, -1, n_quantiles)
                )

                td_error = target_q - current_q
                huber = _quantile_huber_loss(td_error)
                loss = (
                    tau * (td_error > 0).float() * huber
                    + (1 - tau) * (td_error < 0).float() * huber
                ).mean()
