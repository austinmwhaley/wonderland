...
def generate_summary(
    ...
    for env_name, env_results in sorted(all_results.items()):
        ...
        lines.append(f"{'Algorithm':<20} {'Max R':>8} {'Final Avg':>10} {'Time':>8} {'Episodes':>10}  Status")
        lines.append("-" * 80)
        ...
        if all_results[env_name]:
            best_algo = max(env_results.items(), key=lambda kv: kv[1].avg_rewards[-1] if kv[1].avg_rewards else 0)
            lines.append("")
            ...
            if len(dqn_variants) >= 2:
                dqn_scores = {n: np.mean(env_results[n].episode_rewards[-100:]) for n in dqn_variants}
                ...
                if "vanilla_dqn" in dqn_scores and "dqn" in dqn_scores:
                    improvement = ((dqn_scores.get("dqn", 0) - dqn_scores.get("vanilla_dqn", 0)) /
                                   max(abs(dqn_scores.get("vanilla_dqn", 1)), 1)) * 100
            ...
            tabular_variants = [n for n in env_results if n in ('q_learning', 'sarsa', 'expected_sarsa')]
            if len(tabular_variants) >= 2:
                tab_scores = {n: np.mean(env_results[n].episode_rewards[-100:]) for n in tabular_variants}
