...
def main():
    ...
    parser.add_argument(
        "--metric", default="return", help="metric to plot (return, eval_return, regret, loss)"
    )
    parser.add_argument(
        "--ema", type=int, default=20, help="exponential moving average window (0 = raw)"
    )
    parser.add_argument("--loss", action="store_true", help="add a loss subplot")
    parser.add_argument(
        "--out", default=None, help="output png path (default: results/plots/<metric>.png)"
    )
    ...
    if not runs:
        sys.exit(
            "no runs found. Train something first, e.g. "
