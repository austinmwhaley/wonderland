...
def _dummy_records(n=20):
    return [{"pid": f"p{i % 4}", "cat": f"c{i % 3}", "price": float(i * 10)} for i in range(n)]
...
def test_embedding_save_load_transform():
    ...
    emb = EmbeddingModel(
        ...
        config=EmbeddingModelConfig(
            hidden_dim=16, epochs=3, seed=42, device="cpu", sequence_backend="mamba2"
        ),
    )
...
def _classification_records(n=40):
    return [{"cid": f"c{i}", "x": float(i), "label": 1.0 if i < n // 2 else 0.0} for i in range(n)]
...
def test_supervised_regression_save_load_predict():
    torch.manual_seed(42)
    records = [{"cid": f"c{i}", "x": float(i), "label": 2.0 * i} for i in range(20)]
    model = SupervisedModel(
        task="regression",
        id_field="cid",
        target_field="label",
