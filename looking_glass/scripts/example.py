...
def _gen_event_stream() -> list[dict[str, object]]:
    ...
    for cid in range(N_CUSTOMERS):
        ...
        eid += 1
        events.append(_make_event(eid, customer_id, profile_ts, "customer_profile", {
        ...
        for _ in range(n_events):
            ...
            r = RNG.random()
            if r < 0.05:
                events.append(_make_event(eid, customer_id, ts, "web_page_view", {"page_url": f"/shop/{pid}"}))
            elif r < 0.12:
                events.append(_make_event(eid, customer_id, ts, "web_product_view", {"product_id": pid, "page_url": f"/product/{pid}"}))
            elif r < 0.18:
                events.append(_make_event(eid, customer_id, ts, "web_search", {"query": RNG.choice(["shoes", "laptop", "jacket", "book"])}))
            elif r < 0.22:
                events.append(_make_event(eid, customer_id, ts, "web_click", {"element": RNG.choice(["banner", "recommendation", "nav"])}))
            elif r < 0.30:
                events.append(_make_event(eid, customer_id, ts, "add_to_cart", {"product_id": pid}))
            elif r < 0.42:
                loc = RNG.choice(STORE_LOCATIONS)
                payload: dict[str, object] = {"product_id": pid, "order_value": value, "payment_method": RNG.choice(PAYMENT_METHODS)}
                if loc:
                ...
                events.append(_make_event(eid, customer_id, ts, "return_initiated", {"product_id": pid, "return_reason": RNG.choice(["wrong_size", "defective", "changed_mind"])}))
            elif r < 0.52:
                events.append(_make_event(eid, customer_id, ts, "product_review", {"product_id": pid, "rating": RNG.randint(1, 5)}))
            elif r < 0.58:
                events.append(_make_event(eid, customer_id, ts, "email_send", {"campaign_id": RNG.choice([c for c in CAMPAIGN_IDS if c])}))
            elif r < 0.62:
                events.append(_make_event(eid, customer_id, ts, "email_open", {"campaign_id": RNG.choice([c for c in CAMPAIGN_IDS if c])}))
            elif r < 0.66:
                events.append(_make_event(eid, customer_id, ts, "email_click", {"campaign_id": RNG.choice([c for c in CAMPAIGN_IDS if c])}))
            elif r < 0.70:
                events.append(_make_event(eid, customer_id, ts, "sms_send", {"campaign_id": RNG.choice([c for c in CAMPAIGN_IDS if c])}))
            elif r < 0.74:
                events.append(_make_event(eid, customer_id, ts, "sms_click", {"campaign_id": RNG.choice([c for c in CAMPAIGN_IDS if c])}))
            elif r < 0.78:
                events.append(_make_event(eid, customer_id, ts, "app_push_send", {"title": RNG.choice(["Flash Sale!", "New arrivals", "Back in stock"])}))
            elif r < 0.82:
                events.append(_make_event(eid, customer_id, ts, "app_push_open", {}))
            elif r < 0.86:
                events.append(_make_event(eid, customer_id, ts, "app_push_click", {}))
            elif r < 0.90:
                events.append(_make_event(eid, customer_id, ts, "login", {"device": RNG.choice(["ios", "android", "web"])}))
            elif r < 0.93:
                events.append(_make_event(eid, customer_id, ts, "address_update", {}))
            elif r < 0.96:
                events.append(_make_event(eid, customer_id, ts, "support_ticket_created", {"category": RNG.choice(["billing", "shipping", "technical"])}))
            else:
    ...
    for event in events:
        ...
        event["value"] = float(payload.get("order_value", 0.0) or 0.0) if isinstance(payload, dict) else 0.0
...
save_embeddings_to_lancedb(prod_lancedb, "product_embeddings", product_records, "product_id", product_embs)
logger.info("  products=%d  vectors=%d  dim=%d  loss=%.4f", len(product_records), len(product_embs), HIDDEN_DIM, prod_model.loss_)
...
logger.info("  events=%d  customers=%d  event_types=%d", len(events), len({e["customer_id"] for e in events}), len(event_types))
...
product_lookup: dict[str, list[float]] = {
    k: list(v) for k, v in product_embs.vectors.items()
}
...
logger.info("  loss=%.4f  customers=%d  mean_events=%.1f",
...
logger.info("  outcome rows=%d  churn_rate=%.2f  mean_ltv=%.2f",
            len(outcome_frame),
...
def _train_churn(variant_name, numeric_fields, vector_fields):
    ...
    m = create_supervised_model(
        ...
        hidden_dim=HIDDEN_DIM, epochs=HEAD_EPOCHS, seed=SEED,
        learning_rate=1e-2, validation_fraction=0.25, device="cpu",
        sequence_backend="mamba2", pretrained_core=pretrained_core,
    )
    r = m.fit_predict(outcome_rows)
    mets = r.report.metrics
    auc = float(mets["roc_auc"])
    if auc < 0.5:
        logger.warning("  *** churn AUC = %.4f (< 0.5) for variant %s — check for leakage or inverted labels", auc, variant_name)
...
print("╠══════════════════════════════════════════════════════════════════════════════════════════╣")
print(f"║  products        {len(product_records):>5d}   customers  {len(core_outputs.customer_records):>5d}   events  {len(events):>6d}   event types  {len(event_types):>2d}     ║")
print(f"║  outcome rows    {len(outcome_rows):>5d}   core loss  {core_model.loss_:>8.4f}                                          ║")
print("╠══════════════════════════════════════════════════════════════════════════════════════════╣")
print(f"║  churn F1 (vectors)   {churn_vec[0]:>10.4f}   vs xgboost {churn_bl_r.metrics['f1']:>10.4f}   delta {_delta(churn_vec[0], churn_bl_r.metrics['f1']):>10s} ║")
print(f"║  churn F1 (aggrs)    {churn_agg[0]:>10.4f}   vs xgboost {churn_bl_r.metrics['f1']:>10.4f}   delta {_delta(churn_agg[0], churn_bl_r.metrics['f1']):>10s} ║")
print(f"║  churn F1 (combined) {churn_both[0]:>10.4f}   vs xgboost {churn_bl_r.metrics['f1']:>10.4f}   delta {_delta(churn_both[0], churn_bl_r.metrics['f1']):>10s} ║")
print(f"║  churn AUC (vectors) {churn_vec[1]:>10.4f}   vs xgboost {churn_bl_r.metrics['roc_auc']:>10.4f}   delta {_delta(churn_vec[1], churn_bl_r.metrics['roc_auc']):>10s} ║")
print("╠══════════════════════════════════════════════════════════════════════════════════════════╣")
print(f"║  LTV R² (vectors)   {ltv_vec:>10.4f}   vs xgboost {ltv_bl_r.metrics['r2']:>10.4f}   delta {_delta(ltv_vec, ltv_bl_r.metrics['r2']):>10s} ║")
print(f"║  LTV R² (aggrs)     {ltv_agg:>10.4f}   vs xgboost {ltv_bl_r.metrics['r2']:>10.4f}   delta {_delta(ltv_agg, ltv_bl_r.metrics['r2']):>10s} ║")
print(f"║  LTV R² (combined)  {ltv_both:>10.4f}   vs xgboost {ltv_bl_r.metrics['r2']:>10.4f}   delta {_delta(ltv_both, ltv_bl_r.metrics['r2']):>10s} ║")
print("╠══════════════════════════════════════════════════════════════════════════════════════════╣")
print(f"║  churn heads  {churn_time:>5.1f}s  |  LTV heads  {ltv_time:>5.1f}s  |  xgboost  {bl_time:>5.1f}s                                      ║")
print("╚══════════════════════════════════════════════════════════════════════════════════════════╝")
print()
