...
def main():
    ...
    core = create_temporal_core_model(
        ...
        epochs=CORE_EPOCHS, seed=17, learning_rate=CORE_LR,
        device=args.device, sequence_backend="mamba2",
        train_batch_size=64, backbone_version="v1")
    ...
    spec = LabelSpec(id_field="customer_id", timestamp_field="event_ts",
                     as_of=D, history_days=180, horizon_days=HORIZON,
                     value_field="value", label_kind="activity_churn",
    ...
    agg = [f for f in ("event_count", "total_value", "avg_value", "max_value",
                       "recency_days", "recent_count", "recent_value",
                       "active_count") if f in rows[0]]

    def train_churn(name, num, vec):
        m = create_supervised_model(
            task="classification", id_field="customer_id", target_field="churn_label",
            categorical_fields=[], numeric_fields=num, vector_fields=vec,
            hidden_dim=HID, epochs=HEAD_EPOCHS, seed=17, learning_rate=HEAD_LR,
    ...
    m_vec = train_churn("vectors-only", [], ["core_last_vector"])
    m_agg = train_churn("aggregates-only", agg, [])
    m_both = train_churn("combined", agg, ["core_last_vector"])

    if HAS_GBT:
        try:
            bl = GBTBaseline("classification", "customer_id", "churn_label", agg, seed=17)
            r = bl.fit_predict(copy.deepcopy(rows))
            print(f"{'xgb-baseline':16s} F1={r.metrics.get('f1', 0):.3f} AUC={r.metrics.get('roc_auc', 0):.3f}")
