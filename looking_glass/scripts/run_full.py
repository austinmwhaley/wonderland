...
def main() -> None:
    ...
    parser.add_argument(
        "--event-limit", type=int, default=0, help="Cap events for fast smoke runs (0 = all)"
    )
    ...
    logger.info(
        "  products=%d  customers=%d  stores=%d  campaigns=%d  events=%d",
        len(products),
        len(customers),
        len(stores),
        len(campaigns),
    ...
    core_events = [
        e
        for e in events
        if datetime.fromisoformat(str(e["event_ts"]).replace("Z", "+00:00")) <= OUTCOME_CUTOFF
    ]
    ...
    logger.info(
        "  loss=%.4f  customers=%d  mean_events=%.1f  time=%.1fs",
        core_loss,
        len(core_outputs.customer_records),
        sum(float(r["core_event_count"]) for r in core_outputs.customer_records)
    ...
    logger.info(
        "  outcome rows=%d  churn_rate=%.3f  mean_ltv=%.2f",
        len(outcome_frame),
    ...
    t0 = time.perf_counter()
    churn_bl = GBTBaseline(
        "classification", "customer_id", "churn_label", agg_features, seed=SEED
    ).fit_predict(outcome_rows)
    ltv_bl = GBTBaseline(
        "regression", "customer_id", "value_label", agg_features, seed=SEED
    ).fit_predict(outcome_rows)
    ...
    def _warn_auc(auc_val, label):
        if auc_val < 0.5:
            print(
                f"║  *** WARNING: {label} AUC = {auc_val:.4f} (<0.5) — check for leakage/inversion ***  ║"
            )
    ...
    print(
        f"║  products          {len(products):>6d}   stores        {len(stores):>6d}   campaigns      {len(campaigns):>6d}      ║"
    )
    print(
        f"║  customers         {n_customers:>6d}   core events    {len(core_events):>6d}   outcome rows    {len(outcome_rows):>6d}      ║"
    )
    print(
        "╠══════════════════════════════════════════════════════════════════════════════════════╣"
    )
    print(
        f"║  temporal core loss     {core_loss:>8.4f}                                                ║"
    )
    print("╠══════════════════════════╦══════════════╦══════════════╦══════════════╗")
    ...
    for label, met in [
        ("vectors-only  ", churn_vec),
        ("aggregates-only", churn_agg),
        ("combined       ", churn_combined),
    ]:
        f1v = float(met["f1"])
        aucv = float(met.get("roc_auc", 0.5))
        _warn_auc(aucv, label.strip())
        print(
            f"║  {label:<20s}    ║  {f1v:>10.4f}  ║  {aucv:>10.4f}  ║  {_delta(f1v, bl_f1):>12s}  ║"
        )
    ...
    for label, met in [("vectors-only  ", ltv_vec), ("combined       ", ltv_combined)]:
        ...
        print(
            f"║  {label:<20s}    ║  {r2v:>10.4f}  ║  {rmsev:>10.4f}  ║  {_delta(r2v, bl_r2):>12s}  ║"
        )
    print(f"║  xgboost                 ║  {bl_r2:>10.4f}  ║  {bl_rmse:>10.4f}  ║  (baseline)  ║")
    print("╠══════════════════════════╩══════════════╩══════════════╩══════════════╣")
    print(
        f"║  total time  {total_time:>5.0f}s   churn heads {churn_time:>5.1f}s   LTV heads {ltv_time:>5.1f}s   xgboost {bl_time:>5.1f}s        ║"
    )
    print(
        "╚══════════════════════════════════════════════════════════════════════════════════════╝"
    )
