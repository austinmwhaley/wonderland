...
def main():
    ...
    agg = [f for f in ("event_count", "total_value", "avg_value", "max_value",
                       ...
                       "active_count") if f in orows[0]]

    for lr in (1e-2, 3e-3, 1e-3, 3e-4):
        m = create_supervised_model(
            task="classification", id_field="customer_id", target_field="churn_label",
            categorical_fields=[], numeric_fields=agg, vector_fields=[],
            hidden_dim=128, epochs=60, seed=17, learning_rate=lr,
            validation_fraction=0.25, device="cuda", sequence_backend="mamba2")
        res = m.fit_predict(copy.deepcopy(orows))
        met = res.report.metrics
        print(f"lr={lr:<8} F1={met.get('f1', 0):.3f} AUC={met.get('roc_auc', 0):.3f} "
