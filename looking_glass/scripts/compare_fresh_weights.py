...
def train_ltv(rows):
    agg = ("event_count", "total_value", "avg_value", "max_value",
    ...
    m = create_supervised_model(
        ...
        categorical_fields=[], numeric_fields=num,
...
def main():
    ...
    fresh = [e for e in events
             if e["event_ts"] <= d_iso or e["customer_id"] not in eval_set]
    print(f"per-D records={len(per_d)} fresh records={len(fresh)} "
    ...
    eval_ids = set()
    base = [r for r in build_outcomes(events, spec).rows
            if str(r["customer_id"]) in eval_set]
    ...
    rows_a = [dict(r, **look_a[str(r["customer_id"])]) for r in copy.deepcopy(base)
    ...
    rows_b = [dict(r, **look_b[str(r["customer_id"])]) for r in copy.deepcopy(base)
    ...
    print("EVAL vectors contain no own-future in either arm. "
