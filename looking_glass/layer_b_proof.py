...
def _load(name="data/arrow/customer_event_stream.feather",
		  products="artifacts/cfm/cfm_products.duckdb"):
	stream = pl.read_ipc(name, memory_map=True) if str(name).endswith(
		(".arrow", ".feather", ".ipc")) else _read_duckdb(name)
	import duckdb
	con = duckdb.connect(str(products), read_only=True)
	try:
		anch = con.execute("SELECT customer_key, anchor_epoch, version, embedding "
						   "FROM anchor_embeddings").pl()
	finally:
		con.close()
	tag = anch["version"][0]
	...
	try:
		orders = con.execute("SELECT customer_id, epoch(CAST(order_ts AS TIMESTAMPTZ)) t, "
							 "gross_margin gm FROM orders").pl()
...
def build(stream, anch, orders, data_end, window_days):
	...
	for row in anch.iter_rows(named=True):
		...
		o = oby.get(k, [])
		ot = np.array([x[0] for x in o]); ogm = np.array([x[1] for x in o])
		trail = float(ogm[(ot > a - W) & (ot <= a)].sum()) if len(o) else 0.0
		fwd = float(ogm[(ot > a) & (ot <= a + W)].sum()) if len(o) else 0.0
		gap = np.diff(pt)
		rfm = [a - pt[-1], a - pt[0], float(m.sum()), float(pv.sum()),
			   float(np.mean(gap)) if len(gap) else 0.0, float((pe == "order_placed").sum()),
			   trail]
		rfm += [float((pe == t).sum()) for t in etypes]
		E.append(list(row["embedding"])); R.append(rfm)
		y.append(fwd); groups.append(k)
	return (np.array(E, np.float64), np.array(R, np.float64),
			np.array(y, np.float64), np.array(groups))
...
def cv_pred(X, y, groups, folds=5):
	...
	for tr, te in gkf.split(X, y, groups):
		...
		pred[te] = _ridge(sc.transform(X[tr])).fit(sc.transform(X[tr]), y[tr]) \
...
def _partial_ci(pE, pR, y, groups, B=400, seed=0):
	...
	uniq = np.unique(groups)
	idx_by = {k: np.where(groups == k)[0] for k in uniq}
	d = [stat(np.concatenate([idx_by[k] for k in rng.choice(uniq, len(uniq), replace=True)]))
		 for _ in range(B)]
...
def run(windows=(365, 90), seed=0):
	...
	for w in windows:
		...
		pE, pR, pER, pS = (cv_pred(E, y, groups), cv_pred(R, y, groups),
						   cv_pred(ER, y, groups), cv_pred(Scr, y, groups))
		...
		rows.append({"window": w, "n": int(len(y)), "E": rE, "R": rR, "E+R": rER,
					 "scramble": rS, "increment": rER - rR, "ci": (lo, hi),
					 "unique": up, "uci": (ulo, uhi),
					 "signal": signal, "standalone": standalone, "increment_ok": increment})
	return {"version": str(tag), "rows": rows, "success": ok_all}


def show(res):
	print(f"== LAYER B PROOF ==  version={res['version']}  (encoder: self-supervised only)")
	print(f"{'window':>7s} {'n':>5s} {'E':>7s} {'R':>7s} {'scram':>7s} "
		  f"{'unique(E|R)':>12s} {'95% CI':>16s}")
	for r in res["rows"]:
		print(f"{r['window']:>6d}d {r['n']:5d} {r['E']:7.3f} {r['R']:7.3f} {r['scramble']:7.3f} "
			  f"{r['unique']:+12.3f} [{r['uci'][0]:+.3f},{r['uci'][1]:+.3f}]")
	print("\nchecks: S signal E>scramble | C standalone E>=R-0.05 | I unique(E|R) CI_lo>0")
	for r in res["rows"]:
		print(f"  {r['window']:>3d}d: S={'Y' if r['signal'] else 'N'} "
