...
def build_rows(df, anchors, company):
    ...
    for row in anchors.iter_rows(named=True):
        ...
        feats = [
            ...
            float(pv.sum()),
            span,
            float(np.mean(np.diff(pt))) if len(pt) > 1 else 0.0,
            float(np.mean(pv)),
        ]
        ...
        groups.append(k)
    return (
        np.array(Xc, dtype=np.float64),
        np.array(Xr, dtype=np.float64),
        np.array(y_type),
        np.array(y_val, dtype=np.float64),
        np.array(y_dt, dtype=np.float64),
        np.array(groups),
    )
...
def _reg(X, y, groups, seed=0):
    ...
    sc = StandardScaler().fit(X[tr])
    m = Ridge(alpha=1e-3 * float(np.mean(np.diag(X[tr].T @ X[tr]))) + 1e-9).fit(
...
def evaluate(db, products, customers=500):
    ...
    for name, fn, y, dense in tasks:
        ...
        cfm = fn(Xc[m], y[m], groups[m])
        raw = fn(Xr[m], y[m], groups[m])
