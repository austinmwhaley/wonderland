    ...
    with torch.no_grad():
        for seq in seqs:
            ...
            while t <= ts[-1]:
                ...
                if idx >= 0:
                    rows_k.append(seq["customer"])
                    rows_e.append(float(t))
                    rows_c.append(cadence)
                    rows_v.append(y[idx].tolist())
                t += step
    out = pl.DataFrame(
        {"customer_key": rows_k, "epoch": rows_e, "cadence": rows_c, "embedding": rows_v}
    )
    OUT.parent.mkdir(parents=True, exist_ok=True)
    out.write_ipc(str(OUT))
    return {
        "rows": out.height,
        "customers": len(set(rows_k)),
        "dim": len(rows_v[0]) if rows_v else 0,
        "cadence": cadence,
        "out": str(OUT),
    }
