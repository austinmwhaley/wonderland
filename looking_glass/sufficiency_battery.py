...
def build_all(stream, anch, orders, data_end):
	...
	sby = {}
	while i < n:
		k = sc["customer_key"][i]; j = i
		while j < n and sc["customer_key"][j] == k:
			j += 1
		sby[k] = (np.array([_epoch(x) for x in sc["event_ts"][i:j]], dtype=np.float64),
	...
	E, R, groups = [], [], []
	T = {k: [] for k in ("gp_365", "gp_90", "gp_30", "reorder_90", "reorder_30",
						 "orders_90", "days_to_next", "next_order_value")}
	D = 86400.0
	for row in anch.iter_rows(named=True):
		...
		o = oby.get(k, []); ot = np.array([x[0] for x in o]); og = np.array([x[1] for x in o])
		...
		rfm = [a - pt[-1], a - pt[0], float(m.sum()), float(pv.sum()),
			   ...
			   float((pe == "order_placed").sum()), trail]
		rfm += [float((pe == t).sum()) for t in etypes]
		E.append(list(row["embedding"])); R.append(rfm); groups.append(k)
		fut = ot > a
	...
	return (np.array(E, np.float64), np.array(R, np.float64), np.array(groups),
...
def run(threshold=0.6, seed=0):
	...
	for name, y in targets.items():
		...
		rE, rR, rS = (_rho(cv_pred(Xe, yy, gg), yy), _rho(cv_pred(Xr, yy, gg), yy),
					  _rho(cv_pred(Xs, yy, gg), yy))
		up, ulo, uhi = _partial_ci(cv_pred(Xe, yy, gg), cv_pred(Xr, yy, gg), yy, gg)
		rows.append({"target": name, "n": int(mask.sum()), "E": rE, "R": rR, "scram": rS,
					 "unique": up, "uci": (ulo, uhi),
					 "S": rE > rS + 0.05, "C": rE >= rR - 0.05, "I": ulo > 0.0})
	sc = np.mean([r["S"] for r in rows]); cc = np.mean([r["C"] for r in rows])
	ic = np.mean([r["I"] for r in rows])
	success = ic >= threshold and sc >= 0.8 and cc >= 0.8
	return {"version": str(tag), "rows": rows, "n": n, "signal_cov": sc,
			"standalone_cov": cc, "unique_cov": ic, "threshold": threshold,
			"success": bool(success)}


def show(res):
	print(f"== SUFFICIENCY BATTERY ==  version={res['version']}  anchors={res['n']}  "
		  f"(encoder self-supervised only)")
	print(f"{'target':18s} {'n':>5s} {'E':>7s} {'R':>7s} {'scram':>7s} "
		  f"{'unique(E|R)':>12s} {'95% CI':>16s}  S C I")
	for r in res["rows"]:
		print(f"{r['target']:18s} {r['n']:5d} {r['E']:7.3f} {r['R']:7.3f} {r['scram']:7.3f} "
