...
from white_queen import db as wqdb
...
def survey_env(env_name, seed=0, out_dir="/tmp/opencode/wq_survey"):
    ...
    cfg = dict(QUICK_LOOK)
    cfg.update({
        "env": env_name, "seeds": [seed], "collect_episodes": 25,
        "collect_eps": 0.2, "dqn_train_steps": 20000, "pg_train_steps": 20000,
        "checkpoint_fracs": (0.1, 1.0), "db_path": db_path, "ckpt_dir": ckpt,
    })
    ...
    rep = pipeline.run(db_path, gamma=cfg["gamma"], estimate_propensity=True,
                       offline_steps=6000, fast=True, ensemble_K=3,
                       fqe_cfg={"steps_max": 3000, "eval_every": 500,
                                "patience": 6, "batch": 512, "hidden": 96},
    ...
    with open(os.path.join(out_dir, f"verdict_{env_name}.json"), "w") as f:
        json.dump({k: v for k, v in rep.items() if k != "estimates"}, f,
...
if __name__ == "__main__":
    ...
    for e in envs:
        try:
            ...
            for n in r["rank"]:
                est = r["estimates"][n]
                print(f"    {n:8s} FQE={est['fqe_dm']} FQEarg={est['sharp_dm']} "
