...
data=db.load_diet('/home/austin-whaley/wq/white_queen/data/white_queen_quick.db','mixed')
bs=behavior_stats(data,0.99); print('behavior mean',round(bs['mean'],1),'std',round(bs['std'],1), flush=True)
class U:
...
for name, cand in [('uniform',U()), ('iql', load_candidate('iql',env,data,cfg,'/home/austin-whaley/wq/white_queen/verdicts/v15/iql_mixed.pt'))]:
    for steps in (300000,):
        torch.manual_seed(0)
        _,dm,info=E.fit_fqe(data,cand,0.99,{'steps_max':steps,'eval_every':5000,'patience':30,'batch':512,'hidden':128},temperature=1.0)
        print(f'FQE {name} steps_max={steps}: DM={dm:.1f} stopped={info["stopped"]} val={info["val_bellman"]}', flush=True)
# 2) MB with large dynamics budget
step_fn,info=learn_dynamics(data,cfg={'steps_max':50000,'eval_every':2500,'patience':10,'batch':512,'hidden':128},cache_dir=None)
