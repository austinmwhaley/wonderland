...
def _fit_world(s, a, r, nA, tr, seed=0, steps=2000, K=5):
	...
	class M(nn.Module):
		def __init__(self):
			super().__init__()
			self.net = nn.Sequential(nn.Linear(dim + nA + 1, 256), nn.ReLU(),
	...
	S = torch.tensor(s, device=dev); A = torch.tensor(a, device=dev)
	R = torch.tensor(r, device=dev); X = torch.tensor(resid, dtype=torch.float32, device=dev)
	...
	with torch.no_grad():
		...
		for m in ens:
			...
			for ac in range(nA):
				...
				out = m.net(torch.cat([S, oh, torch.tensor(rx, dtype=torch.float32, device=dev)[:, None]], -1))
				rows.append((out * oh).sum(-1))
			grids.append(torch.stack(rows, 0))          # (nA, N)
		grid = torch.stack(grids, 0)                     # (K, nA, N)
	mean = grid.mean(0); std = grid.std(0)
	val_mean = float(mean.max(0).values.mean())
	val_pess = float((mean - 1.0 * std).max(0).values.mean())
	return {"behavior": behavior, "true_greedy": true_greedy,
			"ips_behavior": ips_behavior,
			"wm_value_mean": val_mean, "wm_value_pessimistic": val_pess}
