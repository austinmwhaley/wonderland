...
def main(seed=0):
    ...
    for ra in (0.5, 0.8, 1.0):
        d0 = _judge.judge_diet(rows, b["mean"], b["std"], None, risk_aversion=ra)["decisions"][
            "iql"
        ]
        d1 = _judge.judge_diet(rows2, b["mean"], b["std"], None, risk_aversion=ra)["decisions"][
            "iql"
        ]
        print(
            f"  {ra:>8} {str(d0['deploy']) + ' w=' + str(d0.get('witnesses')):>18} "
