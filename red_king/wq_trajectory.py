...
def main(n_max=8000, seed=0):
    ...
    rep = run(
        src, algorithms=("iql",), nA=nA, fast=True, ensemble_K=2, offline_steps=1500, seed=seed
    )
    print("== WHITE_QUEEN ON TRAJECTORIES ==")
    print(
        "  without red_king -> keys:",
        sorted(list(rep.keys()))[:12] if isinstance(rep, dict) else type(rep),
    )
    ...
    r = ms(seed=seed)
    print(
        f"  WITH red_king multi-step value spearman: {r['rho_mb']:+.3f} (vs model-free {r['rho_mf']:+.3f})"
    )
    return rep
