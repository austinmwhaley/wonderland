...
def _load():
	...
	try:
		anch = pc.execute("SELECT customer_key, anchor_epoch, embedding "
						  "FROM anchor_embeddings ORDER BY customer_key, anchor_epoch").pl()
	finally:
		pc.close()
	sc = duckdb.connect(str(STREAM_DB), read_only=True)
	try:
		sends = sc.execute("SELECT customer_key, epoch(CAST(event_ts AS TIMESTAMPTZ)) t "
...
def build(nA=4):
	...
	while i < n:
		...
		while j < n and rows[j]["customer_key"] == k:
			j += 1
		grp = rows[i:j]
		st = np.array(sby.get(k, [])); io = iby.get(k, [])
		it = np.array([x[0] for x in io]); ig = np.array([x[1] for x in io])
		if len(grp) >= 2:
			for m in range(len(grp) - 1):
				...
				steps.append((grp[m]["embedding"], a, r, grp[m + 1]["embedding"],
							  done, (t1 - t0) / 86400.0, tid))
			tid += 1
		i = j
	...
	return {"n_steps": int(len(S)), "n_traj": int(tid), "dim": int(S.shape[1]),
			"steps_per_traj": round(len(S) / max(tid, 1), 2),
			"reward_mean": float(R.mean()), "reward_nonzero": float((R > 0).mean()),
			"return_mean": float(G.mean()), "out": str(OUT)}
...
def main():
	...
	print("  donor->discounted-return spearman:", round(float(spearmanr(m.predict(sc.transform(S[te])), G[te]).statistic), 3))
