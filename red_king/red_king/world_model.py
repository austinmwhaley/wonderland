...
def _load_anchors():
	...
	try:
		return con.execute("SELECT customer_key, anchor_epoch, embedding "
						   "FROM anchor_embeddings ORDER BY customer_key, anchor_epoch").pl()
	finally:
		con.close()
...
def _load_facts():
	...
	try:
		sends = con.execute(
			...
			"FROM customer_events WHERE event_type='email_send'").pl()
		orders = con.execute(
...
def build_transitions(nA=4):
	...
	while i < n:
		...
		ot = orders_by.get(k, [])
		oga = np.array([x[1] for x in ot]); otime = np.array([x[0] for x in ot])
		for m in range(len(grp) - 1):
			...
			S.append(grp[m]["embedding"]); S2.append(grp[m + 1]["embedding"])
			R.append(r); A.append(a); D.append(0.0); C.append(k)
		i = j
	...
	nAb = int(ab.max()) + 1
	return (np.array(S, np.float32), ab, np.array(R, np.float32),
			np.array(S2, np.float32), np.array(D, np.float32), nAb, np.array(C))


def _split(groups, seed=0, test=0.3):
	from sklearn.model_selection import GroupShuffleSplit
	tr, te = next(GroupShuffleSplit(1, test_size=test, random_state=seed)
...
def train(nA, epochs=None, seed=0):
	...
	class MLP(torch.nn.Module):
		def __init__(self, h=256):
			...
			self.net = torch.nn.Sequential(
				torch.nn.Linear(dim + nAb, h), torch.nn.ReLU(),
	...
	Rt = torch.tensor(Rz, device=dev); S2t = torch.tensor(S2, device=dev)
	dS = S2t - St
	...
	for m, opt in zip(ens, opts):
		...
		for _ in range(steps):
			...
			_, rp, _ = m(St[b], At[b])
			ds, _, _ = m(St[b], At[b])
			loss = torch.nn.functional.mse_loss(rp, Rt[b]) \
				+ 0.1 * torch.nn.functional.mse_loss(ds, dS[b])
	...
	with torch.no_grad():
		St_te = St[tei]
		r_all = torch.stack([torch.stack(
			[m(St_te, torch.full((len(te),), a, device=dev, dtype=torch.long))[1]
			 for a in range(nAb)]) for m in ens])          # (K, nA, N)
		r_grid = r_all.mean(0)                            # (nA, N)
		best_a = r_grid.argmax(0)
	...
	return {"n": int(len(S)), "nA": nAb, "dim": dim, "reward_r2": r2,
			"next_state_cos": cos, "behavior_value": behavior,
