...
def _bandit(n=6000, d=8, nA=4, seed=0, logging_temp=0.5, noise=0.5):
    ...
    mu = (
        base
        + 1.5 * np.sin(S[:, [0]] + np.arange(nA)[None, :] * 0.9)
    ...
    logits = 2.0 * S[:, [0]] * np.array([1.0, -0.5, 0.7, -0.9])[None, :] + rng.normal(
        0, 0.5, (n, nA)
    )
    ...
    with torch.no_grad():
        ...
        for m in ens:
            oh = nn.functional.one_hot(
                torch.arange(nA, device=dev).repeat(len(S), 1).T.reshape(-1), nA
            ).float()
...
def run(seed=0):
    ...
    np.array(
        [float((r[(a == x)] / p[(a == x), x]).mean()) if (a == x).any() else 0.0 for x in range(nA)]
    )
    ...
    return {
        "n": len(S),
        "nA": nA,
        "S1_ordering": round(s1, 3),
        "S2_calibration_ratio": round(s2, 3),
        "S3_rank_acc": round(s3, 3),
        "S4_value_rel_err": round(s4, 3),
        "S5_low_overlap_acc": round(s5, 3),
        "S6_beat_modelfree": round(s6, 3),
        "SCORE": round(score, 3),
    }
...
def real(seed=0):
    ...
    try:
        ...
        pc.close()
    df = (
        arm.join(inc, on="k", how="left")
        .join(emb, left_on="k", right_on="customer_key", how="inner")
    ...
    nA = int(a.max()) + 1
    ipw = np.array(
        [float((g[a == x] / e[a == x]).sum() / max((1 / e[a == x]).sum(), 1e-9)) for x in range(nA)]
    )
    ...
    return {
        "real_ordering": round(float(spearmanr(ipw, imag).statistic), 3),
        "real_calibration_ratio": round(float(imag.mean() / max(ipw.mean(), 1e-9)), 3),
        "real_best_arm_true": int(ipw.argmax()),
        "real_best_arm_red_king": int(imag.argmax()),
        "real_rank_match": bool(int(ipw.argmax()) == int(imag.argmax())),
        "real_per_customer_rank_acc": pc_acc,
        "real_best_arm_dist_true": {
            int(k): int(v) for k, v in zip(*np.unique(opt, return_counts=True))
        }
        if opt is not None
        else {},
