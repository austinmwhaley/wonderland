...
def build_sequences():
    ...
    try:
        arm = sc.execute(
            "SELECT customer_id AS customer_key, epoch(CAST(send_ts AS TIMESTAMPTZ)) t, "
            "arm, propensity FROM email_sends WHERE arm IS NOT NULL"
        ).pl()
        inc = sc.execute("""
    ...
    dim = len(seqs[0][0][0])
    N = len(seqs)
    S = np.zeros((N, T, dim), np.float32)
    A = np.zeros((N, T), np.int64)
    R = np.zeros((N, T), np.float32)
...
def train(seed=0, K=3, zdim=32, hdim=256, steps=4000):
    ...
    ens = []
    for _ in range(K):
        m, zd, hd = make_rssm(nA, dim, zdim, hdim)
        ens.append(m.to(dev))
    ...
    for m, opt in zip(ens, opts):
        ...
        for _ in range(steps):
            ...
            h = torch.zeros(len(b), hdim, device=dev)
            z = torch.zeros(len(b), zdim, device=dev)
            rec = rew = kl = ce = 0.0
            for t in range(Ls):
                s_t = St[b, t]
                ...
                kl_t = 0.5 * (
                    torch.exp(qlv - plv) + (pmu - qmu) ** 2 / torch.exp(plv) - 1 + plv - qlv
                ).sum(-1)
    ...
    with torch.no_grad():
        ...
        h = torch.zeros(len(evt), hdim, device=dev)
        z = torch.zeros(len(evt), zdim, device=dev)
        pr, tr, cc = [], [], []
        for t in range(T):
            s_t = St[evt, t]
            ...
            mk = Mt[evt, t] > 0
            pr.append((r_hat.squeeze(-1) * rstd)[mk])
            tr.append(Rt[evt, t][mk] * rstd)
            cc.append(torch.nn.functional.cosine_similarity(s_hat, s_t, dim=-1)[mk])
            z = qmu
        PR = torch.cat(pr).cpu().numpy()
        TRt = torch.cat(tr).cpu().numpy()
        r2 = 1 - ((PR - TRt) ** 2).sum() / (((TRt - TRt.mean()) ** 2).sum() + 1e-9)
        cosm = float(torch.cat(cc).mean())
    OUT.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "ens": [m.state_dict() for m in ens],
            "hdim": hdim,
            "zdim": zdim,
            "nA": nA,
            "dim": dim,
...
def rollout_arm_values(states, horizon=6, gamma=GAMMA_DAY, path=OUT, batch=512):
    ...
    ens = []
    for sd in blob["ens"]:
        m, _, _ = make_rssm(nA, dim, zdim, hdim)
        m.load_state_dict(sd)
        m.eval()
        ens.append(m)
    states = np.asarray(states, np.float32)
    N = len(states)
    V = np.zeros((N, nA), np.float32)
    SD = np.zeros((N, nA), np.float32)
    with torch.no_grad():
        for a in range(nA):
