...
def build():
    ...
    try:
        snd = sc.execute(
            "SELECT customer_id k, epoch(CAST(send_ts AS TIMESTAMPTZ)) t, arm, propensity "
...
def run(seed=0, K=5, steps=3000):
    ...
    Xtr = torch.tensor(X, device=dev)
    Atr = torch.tensor(A, device=dev)
        ...
        for m in ens:
            ...
            for a in range(nA):
                oh = nn.functional.one_hot(
                    torch.full((len(ks),), a, device=dev, dtype=torch.long), nA
                ).float()
