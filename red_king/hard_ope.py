...
def _red_king_mb(canon, cand, nA, seed=0, K=5, steps=2000):
	...
	St = torch.tensor(obs, device=dev); At = torch.tensor(act, device=dev)
...
def main(argv=None):
	...
	logs, info = make_bandit(n=4000, d=6, nA=4, seed=1, reward_scale=3.0, noise=0.3,
							 logging_temp=a.logging_temp)
	nA = info["nA"]
	...
	panel2 = dict(panel); panel2["mb"] = {"mb": rk_val, "se": rk_se, "sims": 5}
	...
	print(f"  TRUE best(value)     : {info['mean_reward_best']:.3f}  behavior {info['mean_reward_behavior']:.3f}")
	...
	return {"true_best": info["mean_reward_best"], "behavior": info["mean_reward_behavior"],
			"without": dec0["deploy"], "with": dec1["deploy"]}
